...
/*************************************************
 * GET PUBLISHED MEMBERS (for Rete Online section)
 *************************************************/
function getPublishedMembers() {
  return withLock_(() => {
    ...
    return items.map(m => ({
      ...
      citta: m.city || '',
      email: m.email || '',
      foto: m.img ? ('https://amicidiboyle.github.io/images/rete/' + m.img) : '',
      tags: m.tags || []
    }));
  });
}
...
/*************************************************
 * REMOVE MEMBER FROM rete-members.json
 *************************************************/
function removePublishedMember(email) {
  return withLock_(() => {
    ...
    saveReteMembers_(filtered, rete.sha);
...
/*************************************************
 * SYNC PUBLISHED STATUS (optional)
 *************************************************/
function syncPublishedStatus() {
  return withLock_(() => {
    const rete = getReteMembers_();
    ...
    for (let r = 2; r <= lastRow; r++) {
      const email = String(sheet.getRange(r, emailCol).getValue() || '')
        .toLowerCase()
        .trim();

      if (publishedEmails.has(email)) {
        sheet.getRange(r, noteCol).setValue('Pubblicato su rete-members.json');
        updated++;
      }
    }
