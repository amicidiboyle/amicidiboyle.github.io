...
function checkAdminPassword(pass) {
  ...
  return String(pass || '').trim() === String(real || '').trim();
}
...
function removePublishedMember(email) {
  return withLock_(() => {
    ...
    const filtered = items.filter(m => String(m.email || '').trim().toLowerCase() !== cleanEmail);

    if (filtered.length === items.length) {
      throw new Error('Membro non trovato nel file pubblicato');
    }
...
function debugAdminPassword() {
  const real = getAdminPassword_();
  Logger.log('[' + real + ']');
  Logger.log('lunghezza=' + String(real || '').length);
}

function testGithubConnection() {
  const rete = getReteMembers_();
  Logger.log('items=' + JSON.stringify(rete.items));
  Logger.log('sha=' + rete.sha);
}
